...
class SuiteAnalytics:
    ...
    def __init__(self, state: SuiteState):
        self.state = state

    # get the count of the rack generations
    def get_generation_counts(self): # tested
        return Counter(r.generation for r in self.state.positions.values() if r is not None and r.generation != 0)

    # get the count of the rack types
    def get_type_counts(self): # tested
        return Counter(r.type for r in self.state.positions.values() if r is not None)

    # get the sum of the empty rack positions
    def get_empty_positions(self): # tested
        return sum(1 for r in self.state.positions.values() if r is None)
    ...
    def get_row_distribution(self): # tested
        return Counter(pos.row for pos, rack in self.state.positions.items() if rack is not None)

    # counts all 2023 racks
    def get_2023_count(self): # tested
        return sum(1 for r in self.state.positions.values() if r is not None and r.generation == 2023)

    # gets the rsu per service
    def get_rsu_per_service(self): # tested

        temp = {}
        for r in self.state.positions.values():
    ...
    def total_power_kw(self) -> int: # tested
        return sum(r.powerNeed for r in self.state.positions.values() if r is not None)
