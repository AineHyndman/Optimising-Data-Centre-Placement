...
class Constraints:
    ...
    def allowed_power_kw(self) -> int:
        return max_power_kw + (emergency_power_kw if not self.suite.emergencyState.cooldown else 0)
    ...
    def check_rsu_max(self, type: str):
        ...
        if type == "AI":
            return round(AI_max - myDict[type], 2)
        elif type == "Storage":
            return round(storage_max - myDict[type], 2)
        elif type == "Compute":
            return round(compute_max - myDict[type], 2)
        else:
            myDict["AI"] = round(AI_max - myDict["AI"], 2)
            myDict["Storage"] = round(storage_max - myDict["Storage"], 2)
            myDict["Compute"] = round(compute_max - myDict["Compute"], 2)
            return myDict
    ...
    def check_rsu_min(self, type: str):
        myDict = self.suite.get_rsu_per_service()
        if type == "AI":
            return round(AI_min - myDict[type], 2)
        elif type == "Storage":
            return round(storage_min - myDict[type], 2)
        elif type == "Compute":
            return round(compute_min - myDict[type], 2)
        else:
            myDict["AI"] = round(AI_min - myDict["AI"], 2)
            myDict["Storage"] = round(storage_min - myDict["Storage"], 2)
            myDict["Compute"] = round(compute_min - myDict["Compute"], 2)
            return myDict
        # Return an Int if type is correctly inputed
